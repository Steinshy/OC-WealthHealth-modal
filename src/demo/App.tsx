...
export default function App() {
  ...
  const storybookHref = import.meta.env.DEV ? 'http://localhost:6006/' : `${import.meta.env.BASE_URL}storybook/`;
  ...
  return (
    <div className={styles.container}>
      ...
      <main className={styles.main}>
        <section className={styles.section} aria-labelledby="storybook-heading">
          <h2 id="storybook-heading">Storybook</h2>
          <p className={styles.storybookNote}>
            Browse components in isolation with docs, controls, and the accessibility addon.
            {import.meta.env.DEV ? (
              <>
                {' '}
                <code className={styles.storybookCode}>npm run dev</code> starts this demo and Storybook together (ports 5173 and 6006). Use{' '}
                <code className={styles.storybookCode}>npm run dev:demo</code> for the demo only, or <code className={styles.storybookCode}>npm run storybook</code> for Storybook only.
              </>
            ) : (
              <>
                {' '}
                On GitHub Pages, Storybook is published next to this demo at <code className={styles.storybookCode}>{import.meta.env.BASE_URL}storybook/</code>.
              </>
            )}
          </p>
          <div className={styles.controls}>
            <a className={styles.button} href={storybookHref} target="_blank" rel="noopener noreferrer">
              Open Storybook
            </a>
          </div>
        </section>
